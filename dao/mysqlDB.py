...
import config
from utils import log_sls, DataEncoder
...
class Mysql:
    ...
    def execute(self, sql, args: list, *,
                to_json: bool = True, raise_error: bool = True, log_key: str = 'mysql', ):
        """
        ...
        with self.__conn as conn:
            try:
                ...
                if sql.startswith('SELECT') or sql.startswith('select'):
                    ...
                    if to_json:
                        result = json.loads(json.dumps(result, cls=DataEncoder.ObjEncoder))
                else:
                    if not args:
                ...
                res['result'] = str(e)
                if raise_error:
                    raise exceptions.DbException(str(e))
                else:
                    log_sls.error(log_key, 'sql执行异常', e=str(e))
            ...
            try:
                ...
                for sql_with_args in sql_with_args_list:
                    ...
                    if sql.startswith('SELECT'):
                        ...
                        if to_json:
                            result_ = json.loads(json.dumps(result_, cls=DataEncoder.ObjEncoder))
